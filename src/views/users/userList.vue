...
import { validateUser, validatePass, validateEmail, validateTel } from '../../utils/validator';
...
// # 添加用户，或者编辑用户，根据flag判断
const addUser = () => {
	if (editUserFlag) {
		userApi.editUser(userForm.value, userId.value).then(res => {
			getUserList()
		...
		userFormRef.value.validate(valid => {
			if (valid) {
				userApi.addUser(userForm.value).then(res => {
					// console.log(res)
					getUserList()
					dialogFormVisible.value = false
				})
...
<template>
	<div class="user">
		<div class="input">
			<el-input v-model="inputValue" style="max-width: 600px;margin-right: 20px;"
		...
		<el-pagination background v-model:page-size="page.pagesize" layout="prev, pager, next"
									 v-model:total="total" v-model:current-page="page.pagenum" />

		<el-dialog v-model="dialogFormVisible" :title="editUserFlag ? '编辑信息' : '添加用户'"
							 width="500" @closed="editUserFlag = false">
